...
class AccuracyCallback(tf.keras.callbacks.Callback):
    ...
    def plot_graph(self):
        ...
        plt.plot(self.train_acc,
                 label='РўРѕС‡РЅРѕСЃС‚СЊ РЅР° РѕР±СѓС‡Р°С‰РµР№ РІС‹Р±РѕСЂРєРµ')
        plt.plot(
            self.val_acc, label='РўРѕС‡РЅРѕСЃС‚СЊ РЅР° РїСЂРѕРІРµСЂРѕС‡РЅРѕР№ РІС‹Р±РѕСЂРєРµ')
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if logs['val_accuracy'] > self.accuracymax:
            ...
            self.idxmax = epoch
        print(f'Р­РїРѕС…Р° {epoch+1}'.ljust(10) + f'Р’СЂРµРјСЏ РѕР±СѓС‡РµРЅРёСЏ: {t}c'.ljust(25) + f'РўРѕС‡РЅРѕСЃС‚СЊ РЅР° РѕР±СѓС‡Р°СЋС‰РµР№ РІС‹Р±РѕСЂРєРµ: {bcolors.OKBLUE}{round(
            logs["accuracy"]*100, 1)}%{bcolors.ENDC}'.ljust(50) + f'РўРѕС‡РЅРѕСЃС‚СЊ РЅР° РїСЂРѕРІРµСЂРѕС‡РЅРѕР№ РІС‹Р±РѕСЂРєРµ: {bcolors.OKBLUE}{round(logs["val_accuracy"]*100, 1)}%{bcolors.ENDC}')
        self.cntepochs += 1
    ...
    def on_train_end(self, logs):
        ...
        for i in range(self.cntepochs):
            if i == self.idxmax:
                print('\33[102m' + f'Р­РїРѕС…Р° {i+1}'.ljust(10) + f'Р’СЂРµРјСЏ РѕР±СѓС‡РµРЅРёСЏ: {self.times[i]}c'.ljust(25) + f'РўРѕС‡РЅРѕСЃС‚СЊ РЅР° РѕР±СѓС‡Р°СЋС‰РµР№ РІС‹Р±РѕСЂРєРµ: {
                      round(self.train_acc[i]*100, 1)}%'.ljust(41) + f'РўРѕС‡РЅРѕСЃС‚СЊ РЅР° РїСЂРѕРІРµСЂРѕС‡РЅРѕР№ РІС‹Р±РѕСЂРєРµ: {round(self.val_acc[i]*100, 1)}%' + '\033[0m')
            else:
                print(f'Р­РїРѕС…Р° {i+1}'.ljust(10) + f'Р’СЂРµРјСЏ РѕР±СѓС‡РµРЅРёСЏ: {self.times[i]}c'.ljust(25) + f'РўРѕС‡РЅРѕСЃС‚СЊ РЅР° РѕР±СѓС‡Р°СЋС‰РµР№ РІС‹Р±РѕСЂРєРµ: {bcolors.OKBLUE}{round(
                    self.train_acc[i]*100, 1)}%{bcolors.ENDC}'.ljust(50) + f'РўРѕС‡РЅРѕСЃС‚СЊ РЅР° РїСЂРѕРІРµСЂРѕС‡РЅРѕР№ РІС‹Р±РѕСЂРєРµ: {bcolors.OKBLUE}{round(self.val_acc[i]*100, 1)}%{bcolors.ENDC}')
        self.plot_graph()
...
class TerraDataset:
    ...
    def load(self):
        ...
        print(f'{bcolors.BOLD}Р—Р°РіСЂСѓР·РєР° РґР°С‚Р°СЃРµС‚Р°{
              bcolors.ENDC}', end=' ')
    ...
    def samples(self):
        ...
        if self.base['task_type'] == 'img_classification':
            ...
            f, ax = plt.subplots(len(self.classes), 5,
                                 figsize=(24, len(self.classes) * 4))
    ...
    def create_sets(self):
        ...
        x_test = []
        y_test = []
        print(f'{bcolors.BOLD}РЎРѕР·РґР°РЅРёРµ РЅР°Р±РѕСЂРѕРІ РґР°РЅРЅС‹С… РґР»СЏ РѕР±СѓС‡РµРЅРёСЏ РјРѕРґРµР»Рё{
              bcolors.ENDC}', end=' ')
        ...
        if self.base['task_type'] == 'img_classification':
            ...
            for j, d in enumerate(self.classes):
                ...
                files = sorted(os.listdir(
                    os.path.join(self.base['dir_name'], d)))
                ...
                for i in range(len(files)):
                    ...
                    if i < count:
                        ...
                        y_test.append(j)
            self.sets = (np.array(x_train)/255., np.array(y_train)
                         ), (np.array(x_test)/255., np.array(y_test))

            # Р’С‹РІРѕРґ С„РёРЅР°Р»СЊРЅРѕР№ РёРЅС„РѕСЂРјР°С†РёРё
            print(f'{bcolors.OKGREEN}Ok{bcolors.ENDC}')
            print()
            print(f'Р Р°Р·РјРµСЂ СЃРѕР·РґР°РЅРЅС‹С… РІС‹Р±РѕСЂРѕРє:')
            print(f'  РћР±СѓС‡Р°СЋС‰Р°СЏ РІС‹Р±РѕСЂРєР°: {
                  self.sets[0][0].shape}')
            print(f'  РњРµС‚РєРё РѕР±СѓС‡Р°СЋС‰РµР№ РІС‹Р±РѕСЂРєРё: {
                  self.sets[0][1].shape}')
            print(f'  РџСЂРѕРІРµСЂРѕС‡РЅР°СЏ РІС‹Р±РѕСЂРєР°: {
                  self.sets[1][0].shape}')
            print(f'  РњРµС‚РєРё РїСЂРѕРІРµСЂРѕС‡РЅРѕР№ РІС‹Р±РѕСЂРєРё: {
                  self.sets[1][1].shape}')
            print()
...
class TerraModel:
    ...
    def create_model(self, layers):
        ...
        inp = self.create_layer(
            f'РІС…РѕРґРЅРѕР№-{self.trds.sets[0][0].shape[1:]}')
    ...
    def test_model(self):
        ...
        if self.task_type == 'img_classification':
            for i in range(10):
                ...
                # Р’С‹РІРѕРґРёРј РёР·РѕР±СЂР°Р¶РµРЅРёРµ РёР· С‚РµСЃС‚РѕРІРѕРіРѕ РЅР°Р±РѕСЂР° СЃ Р·Р°РґР°РЅРЅС‹Рј РёРЅРґРµРєСЃРѕРј
                plt.imshow(sample)
                ...
                for i in range(len(self.trds.classes)):
                    ...
                    print(f'РњРѕРґРµР»СЊ СЂР°СЃРїРѕР·РЅР°Р»Р° РєР»Р°СЃСЃ В«{
                          self.trds.classes[i]}В» РЅР° {round(100*pred[i], 1)}%{bcolors.ENDC}')
                print('---------------------------')
                ...
                print(self.trds.classes[self.trds.sets[1]
                      [1][number]], end=f'{bcolors.ENDC}\n')
                print('---------------------------')
                print()
                print()
...
class TerraIntensive:
    ...
    def create_model(self, layers):
        print(f'{bcolors.BOLD}РЎРѕР·РґР°РЅРёРµ РјРѕРґРµР»Рё РЅРµР№СЂРѕРЅРЅРѕР№ СЃРµС‚Рё{
              bcolors.ENDC}', end=' ')
    ...
    def train_model_average(self, layers, cnt=10):
        if self.task_type == 'img_classification':
            print(f'{bcolors.BOLD}РћРїСЂРµРґРµР»РµРЅРёРµ СЃСЂРµРґРЅРµРіРѕ РїРѕРєР°Р·Р°С‚РµР»СЏ С‚РѕС‡РЅРѕСЃС‚Рё РјРѕРґРµР»Рё РЅР° {
                  cnt} Р·Р°РїСѓСЃРєР°С…{bcolors.ENDC}')
            print()
            average_accuracy = []
            ...
            for i in range(cnt):
                ...
                times.append(t)
                print(f'Р—Р°РїСѓСЃРє {i+1}'.ljust(10) + f'Р’СЂРµРјСЏ РѕР±СѓС‡РµРЅРёСЏ: {t}c'.ljust(25) + f'РўРѕС‡РЅРѕСЃС‚СЊ РЅР° РѕР±СѓС‡Р°СЋС‰РµР№ РІС‹Р±РѕСЂРєРµ: {bcolors.OKBLUE}{round(
                    average_accuracy[-1]*100, 1)}%{bcolors.ENDC}'.ljust(50) + f'РўРѕС‡РЅРѕСЃС‚СЊ РЅР° РїСЂРѕРІРµСЂРѕС‡РЅРѕР№ РІС‹Р±РѕСЂРєРµ: {bcolors.OKBLUE}{round(average_val_accuracy[-1]*100, 1)}%{bcolors.ENDC}')
                gc.collect()
            ...
            for i in range(cnt):
                if i == argmax_idx:
                    print('\33[102m' + f'Р—Р°РїСѓСЃРє {i+1}'.ljust(10) + f'Р’СЂРµРјСЏ РѕР±СѓС‡РµРЅРёСЏ: {times[i]}c'.ljust(25) + f'РўРѕС‡РЅРѕСЃС‚СЊ РЅР° РѕР±СѓС‡Р°СЋС‰РµР№ РІС‹Р±РѕСЂРєРµ: {
                          round(average_accuracy[i]*100, 1)}%'.ljust(41) + f'РўРѕС‡РЅРѕСЃС‚СЊ РЅР° РїСЂРѕРІРµСЂРѕС‡РЅРѕР№ РІС‹Р±РѕСЂРєРµ: {round(average_val_accuracy[i]*100, 1)}%' + '\033[0m')
                else:
                    print(f'Р—Р°РїСѓСЃРє {i+1}'.ljust(10) + f'Р’СЂРµРјСЏ РѕР±СѓС‡РµРЅРёСЏ: {times[i]}c'.ljust(25) + f'РўРѕС‡РЅРѕСЃС‚СЊ РЅР° РѕР±СѓС‡Р°СЋС‰РµР№ РІС‹Р±РѕСЂРєРµ: {bcolors.OKBLUE}{round(
                        average_accuracy[i]*100, 1)}%{bcolors.ENDC}'.ljust(50) + f'РўРѕС‡РЅРѕСЃС‚СЊ РЅР° РїСЂРѕРІРµСЂРѕС‡РЅРѕР№ РІС‹Р±РѕСЂРєРµ: {bcolors.OKBLUE}{round(average_val_accuracy[i]*100, 1)}%{bcolors.ENDC}')
            print()
            print(f'{bcolors.BOLD}РЎСЂРµРґРЅСЏСЏ С‚РѕС‡РЅРѕСЃС‚СЊ РЅР° РѕР±СѓС‡Р°СЋС‰РµР№ РІС‹Р±РѕСЂРєРµ: {
                  bcolors.ENDC}{round(np.mean(average_accuracy[i])*100, 1)}%')
            print(f'{bcolors.BOLD}РњР°РєСЃРёРјР°Р»СЊРЅР°СЏ С‚РѕС‡РЅРѕСЃС‚СЊ РЅР° РѕР±СѓС‡Р°СЋС‰РµР№ РІС‹Р±РѕСЂРєРµ: {
                  bcolors.ENDC}{round(np.max(average_accuracy[i])*100, 1)}%')
            print(f'{bcolors.BOLD}РЎСЂРµРґРЅСЏСЏ С‚РѕС‡РЅРѕСЃС‚СЊ РЅР° РїСЂРѕРІРµСЂРѕС‡РЅРѕР№ РІС‹Р±РѕСЂРєРµ: {
                  round(np.mean(average_val_accuracy[i])*100, 1)}%')
            print(f'{bcolors.BOLD}РњР°РєСЃРёРјР°Р»СЊРЅР°СЏ С‚РѕС‡РЅРѕСЃС‚СЊ РЅР° РїСЂРѕРІРµСЂРѕС‡РЅРѕР№ РІС‹Р±РѕСЂРєРµ: {
                  round(np.max(average_val_accuracy[i])*100, 1)}%')
